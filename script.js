...
function sendData() {
  ...
  urlEncodedDataPairs.push(
    encodeURIComponent("checkin") +
      "=" +
      encodeURIComponent(form.querySelector("[name='checkin']").value)
  );
  urlEncodedDataPairs.push(
    encodeURIComponent("checkout") +
      "=" +
      encodeURIComponent(form.querySelector("[name='checkout']").value)
  );
  urlEncodedDataPairs.push(
    encodeURIComponent("guests") +
      "=" +
      encodeURIComponent(form.querySelector("[name='guests']").value)
  );

  // radio buttons
  let radio = document.getElementsByName("roomtype");
  for (var i = 0, length = radio.length; i < length; i++) {
    if (radio[i].checked) {
      urlEncodedDataPairs.push(
        encodeURIComponent("roomtype") + "=" + encodeURIComponent(radio[i].value)
      );
    }
  }

  // dropdown menu
  var dropdown = form.querySelector("[name='packagetype']");
  urlEncodedDataPairs.push(
  ...
  XHR.addEventListener("load", function (event) {
    if (XHR.readyState === XHR.DONE) {
      if (XHR.status === 200) {
        alert("Your reservation has been received! A confirmation will be sent to your email.");
      } else {
        alert("Oh no! We have a problem! " + XHR.responseText + ".");
      }
    }
  });
